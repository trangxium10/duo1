...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // helper: sha256 hash for storing passwords (SHA-2 family)
    function sha2_hash(string $pwd): string {
    ...
    if (isset($_SESSION['user']) && $_SESSION['user'] && $submit_action === 'update_profile') {
        ...
        if ($submit_action === 'update_profile') {
            ...
            if ($email) {
                ...
                if ($other) {
                    ...
                    if ($newpass !== '') {
                        // For the owner account we keep the original raw password (owner exception).
                        // For all other users, store SHA-2 hashed password.
                        $owner_email = 'xium10@gmail.com';
                        $owner_nom = 'NGUYEN';
                        $owner_prenom = 'Xiu';
                        $is_owner_session = (isset($_SESSION['user']['email']) && strtolower($_SESSION['user']['email']) === strtolower($owner_email)
                            && isset($_SESSION['user']['nom']) && strtoupper($_SESSION['user']['nom']) === strtoupper($owner_nom)
                            && isset($_SESSION['user']['prenom']) && $_SESSION['user']['prenom'] === $owner_prenom);
                        if ($is_owner_session) {
                            $storedPass = $newpass;
                        } else {
                            $storedPass = sha2_hash($newpass);
                        }
                        $upd = $pdo->prepare('UPDATE utilisateurs SET nom = :nom, prenom = :prenom, email = :email, motdepasse = :motdepasse WHERE id = :id');
                        $upd->execute([':nom'=>$nom, ':prenom'=>$prenom, ':email'=>$email, ':motdepasse'=>$storedPass, ':id'=>$userId]);
                    } else {
        ...
        if (!$email || !$motdepasse) {
            ...
            if ($submit_action === 'login') {
                // Accept bcrypt, SHA-2 (sha256) and plain-text stored passwords.
                $ok = false;
                if ($exist) {
                    // bcrypt (password_hash) check
                    if (password_verify($motdepasse, $exist['motdepasse'])) {
                        $ok = true;
                    }
                    // SHA-256 check
                    if (!$ok && hash('sha256', $motdepasse) === $exist['motdepasse']) {
                        $ok = true;
                    }
                    // plain-text fallback (owner or legacy)
                    if (!$ok && $motdepasse === $exist['motdepasse']) {
                        $ok = true;
                    }
                }
                ...
                if ($exist) {
                    ...
                    // Determine whether this new account matches the special owner account.
                    // If it matches the owner identity, store the raw password (owner exception).
                    $owner = [
                        'nom' => 'NGUYEN',
                        'prenom' => 'Xiu',
                    ...
                    $is_owner_identity = (isset($nom, $prenom, $email)
                        && strtoupper($nom) === strtoupper($owner['nom'])
                        && $prenom === $owner['prenom']
                        && strtolower($email) === strtolower($owner['email']));

                    if ($is_owner_identity && $motdepasse === $owner['motdepasse']) {
                        $stored = $motdepasse;
                    } else {
                        // store SHA-256 of password for new users
                        $stored = sha2_hash($motdepasse);
                    }
...
<input type="password" name="motdepasse" required>
    ...
    <button class="btn-principal" type="submit" name="submit_action" value="register">S'inscrire prémium</button>
