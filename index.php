...
$postsLifestyle = [];
if ($pdo) {
    $stmt = $pdo->prepare("SELECT p.title, p.content, p.created_at, u.nom, u.prenom FROM posts p LEFT JOIN utilisateurs u ON p.user_id = u.id WHERE p.category = :cat ORDER BY p.created_at DESC LIMIT 3");
